        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {PAIN_POINTS.map((item, index) => (
            <article
              key={item.title}
              data-reveal
              className="reveal-base rounded-[28px] border border-rose-200 bg-rose-50/70 p-6 transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_40px_rgba(244,63,94,0.12)]"
              style={{ transitionDelay: `${index * 70}ms` }}
            >
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-rose-500 shadow-[var(--shadow-sm)]">
                <XCircle size={22} />
              </div>
              <h3 className="mt-5 text-xl font-bold text-[var(--text-primary)]">{item.title}</h3>
