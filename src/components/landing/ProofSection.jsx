...
export default function ProofSection() {
  return (
    <section className="bg-[var(--bg-gray)] px-4 py-16 lg:px-8 lg:py-24">
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {STATS.map((stat, index) => (
            <div
              key={stat.label}
              data-reveal
              className="reveal-base rounded-[28px] bg-white p-6 shadow-[var(--shadow-sm)]"
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-3">
          {TESTIMONIALS.map((item, index) => (
            <article
              key={item.name}
              data-reveal
              className="reveal-base rounded-[30px] border border-[var(--border)] bg-white p-6 shadow-[var(--shadow-sm)]"
            ...
            >
              ...
              <p className="mt-5 text-lg leading-8 text-[var(--text-primary)]">"{item.quote}"</p>
