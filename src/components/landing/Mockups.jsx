  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 border-b border-[var(--border)] bg-slate-50 px-4 py-3">
        ...
        <div className="ml-3 h-9 flex-1 rounded-full border border-slate-200 bg-white px-4 text-xs leading-9 text-slate-400">
      ...
      <div className="grid gap-0 bg-[var(--bg-gray)] lg:grid-cols-[168px_minmax(0,1fr)]">
        <aside className="border-r border-[var(--border)] bg-slate-950 px-4 py-5 text-white">
          <div className="mb-6 flex items-center gap-2">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-white/10 text-lg">🖨️</div>
            <div>
...
export function FeatureMockup({ variant }) {
    ...
    hpp: (
      <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_180px]">
          ...
          <div className="rounded-2xl bg-slate-50 p-4">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              {['Ukuran Box', 'Gramatur Kertas', 'Biaya Cetak', 'Finishing'].map((item) => (
                <div key={item} className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-500">
                  {item}
                </div>
              ))}
            </div>
          </div>
          <div className="rounded-2xl bg-[var(--brand-blue-light)] p-4">
            <p className="text-xs uppercase tracking-[0.2em] text-[var(--brand-blue)]">Hasil</p>
            <div className="mt-3 flex items-end justify-between">
              <div>
                <p className="text-sm text-slate-600">HPP per pcs</p>
                <p className="text-3xl font-bold text-slate-950">Rp 1.250</p>
              </div>
              <div className="rounded-full bg-white px-3 py-1 text-sm font-semibold text-emerald-600">
                Margin 32%
              </div>
            </div>
          </div>
        ...
        <div className="rounded-2xl bg-slate-950 p-4 text-white">
          <p className="text-sm font-semibold">Rekomendasi Jual</p>
          <p className="mt-3 text-4xl font-bold">Rp 1.650</p>
          <p className="mt-3 text-sm text-slate-300">Masih aman untuk diskon, tetap untung di volume besar.</p>
        </div>
      </div>
    ),
    cutting: (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">32 potong per lembar, waste 6%</p>
            </div>
            <div className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-600">
              Paling Hemat
            </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <div className="flex items-start justify-between border-b border-slate-100 pb-4">
            <div>
              ...
              <p className="text-sm text-slate-500">INV-2026-0423</p>
            </div>
            <div className="rounded-full bg-[var(--brand-blue-light)] px-3 py-1 text-xs font-semibold text-[var(--brand-blue)]">
              Pending
            </div>
          </div>
          <div className="space-y-3 py-4 text-sm text-slate-600">
            <div className="flex justify-between"><span>Box Hampers Lebaran</span><strong>Rp 12.500.000</strong></div>
            <div className="flex justify-between"><span>Diskon</span><strong>- Rp 500.000</strong></div>
            <div className="flex justify-between"><span>PPN</span><strong>Rp 1.320.000</strong></div>
          </div>
          <div className="flex items-center justify-between rounded-2xl bg-slate-950 px-4 py-4 text-white">
            <span>Total Tagihan</span>
            <strong className="text-xl">Rp 13.320.000</strong>
          </div>
        </div>
      </div>
    ),
    tracking: (
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        <div className="mb-5 flex items-center justify-between">
          <div>
            <p className="text-sm font-semibold text-slate-900">Tracking Order Publik</p>
            <p className="text-sm text-slate-500">Order: Dus Kue Premium</p>
          </div>
          <div className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-600">
            Real-time
          </div>
        ...
        <div className="grid gap-3">
          {[
            ...
            <div key={label} className="flex items-center gap-3 rounded-2xl bg-slate-50 px-4 py-3">
              ...
              <span className={`text-sm ${done ? 'font-semibold text-slate-900' : 'text-slate-400'}`}>{label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <div className="rounded-2xl bg-white p-5 shadow-[var(--shadow-sm)]">
          <div className="mb-5 rounded-2xl bg-slate-950 p-5 text-white">
            ...
            <p className="mt-2 text-2xl font-bold">app.naikcetak.com/printbandung</p>
            <p className="mt-2 text-sm text-slate-300">Kemasan custom, sticker, paper bag, dan offset untuk brand lokal.</p>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            {['Dus Kemasan', 'Sticker Label', 'Paper Bag', 'Katalog Produk'].map((item) => (
              <div key={item} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <p className="font-semibold text-slate-900">{item}</p>
                <p className="mt-1 text-sm text-slate-500">Mulai dari Rp 750/pcs</p>
              </div>
            ))}
          </div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
          ...
          <div className="mt-4 rounded-2xl bg-[var(--brand-blue)] px-4 py-3 text-sm font-semibold text-white">
        ...
        <div className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-3">
            {[
              ...
              <div key={label} className="rounded-2xl bg-white p-4 shadow-[var(--shadow-sm)]">
                <p className="text-sm text-slate-500">{label}</p>
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_260px]">
        ...
        <div className="rounded-2xl bg-slate-950 p-5 text-white">
          ...
          <div className="mt-5 rounded-2xl bg-white px-4 py-4 text-slate-950">
            <p className="text-sm text-slate-500">Harga jual rekomendasi</p>
            <p className="mt-1 text-2xl font-bold">Rp 14.365.000</p>
          </div>
        </div>
      </div>
    ),
  ...
  return (
    <BrowserFrame>
      <div className="bg-[var(--bg-gray)] p-4">{variants[variant] ?? variants.dashboard}</div>
    </BrowserFrame>
  );
}
